...
fem_adult <- sum(incarceration_trends$female_adult_jail_pop, na.rm = TRUE)
mal_adult <- sum(incarceration_trends$male_adult_jail_pop, na.rm = TRUE)
fem_juv <- sum(incarceration_trends$female_juvenile_jail_pop, na.rm = TRUE)
mal_juv <- sum(incarceration_trends$male_juvenile_jail_pop, na.rm = TRUE)
...
total_incarceration_FL <- incarceration_trends %>%
  select(state) %>%
  filter(state == "FL") %>%
  summarise(total_pop == max(total_jail_pop, na.rm = TRUE)) %>%
  filter(total_pop == max(total_pop)) %>%
  pull(total_pop)
...
native_jail_pop_time <- ggplot(data = incarceration_trends) +
  geom_col(mapping = aes(x = year , y = native_jail_pop, fill = year)) +
...
female_vs_male <- incarceration_trends %>%
  ...
  summarize(female_adult_jail_pop = mean(female_adult_jail_pop, na.rm = TRUE),
            male_adult_jail_pop = mean(male_adult_jail_pop, na.rm = TRUE))
